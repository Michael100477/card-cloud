...
async function persist(key: string, items: PrivacySection[]) {
...
export function PrivacySectionEditor({ privacyKey, initialValue }: { privacyKey: string; initialValue: string }) {
  const [items, setItems] = useState<PrivacySection[]>(() => {
    try {
      const parsed = JSON.parse(initialValue || "[]");
      if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    } catch { /* fall through */ }
    return PRIVACY_DEFAULTS.map((s, i) => ({ id: String(i), title: s.title, body: s.body, show: true }));
  });
  const [saving, setSaving] = useState(false);
  const [saved,  setSaved]  = useState(false);

  async function save(updated: PrivacySection[]) {
    setSaving(true);
    await persist(privacyKey, updated);
    setSaving(false);
    setSaved(true);
