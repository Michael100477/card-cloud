...
export async function POST(req: NextRequest) {
  ...
  // Use the Trading API EndItem call — simplest path since we have the ebayListingId
...
<EndItemRequest xmlns="urn:ebay:apis:eBLBaseComponents">
  ...
  if (ack !== "Success" && ack !== "Warning") {
    const errMsg = text.match(/<LongMessage>([^<]+)<\/LongMessage>/)?.[1]
                ...
                ?? "Unknown error";
    logger.error({ category: "ebay", action: "ebay.end.failed", message: `EndItem failed: ${errMsg}`, data: { ack, itemId: listing.ebayListingId, response: text.slice(0, 500) }, targetId: listingDbId, targetType: "listing" });
  ...
  // End any active Exchange listing tied to this consignment item
  await db.exchangeListing.updateMany({
    where: { consignmentItemId: listing.item.id, status: "active" },
    data:  { status: "ended" },
  }).catch(() => {});

  revalidatePath("/admin/consignments");
  revalidatePath(`/admin/consignments/${listing.item.orderId}`);
  revalidatePath("/admin/listings");

  logger.info({ category: "ebay", action: "ebay.listing.ended", message: `Listing ended: ${listing.item.player ?? "item"}`, targetId: listingDbId, targetType: "listing" });
