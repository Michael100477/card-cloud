...
export async function POST(req: NextRequest) {
  ...
  if (listing.status === "active") {
    return NextResponse.json({ error: "Already listed on eBay" }, { status: 400 });
  }
  ...
  if (!result.ok) {
    // Persist the error to the listing record so it survives page refresh
    await db.ebayListing.update({
      where: { id: listingDbId },
      data:  { lastError: result.error ?? "Unknown error" },
    });
    console.error(`[ebay/list] FAILED listingDbId=${listingDbId} itemId=${item.id}:\n${result.error}`);
    logger.error({
  ...
  // Clear any previous error and save the real eBay listing ID and URL
  await db.ebayListing.update({
    where: { id: listingDbId },
    data: {
  ...
  // Dual-list on The Exchange — fetch commission rate then create ExchangeListing
