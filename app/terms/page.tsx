...
export default async function TermsPage() {
  ...
  let customSections: TermsSection[] = [];
  try { customSections = JSON.parse(cms["terms_sections"] || "[]").filter((s: TermsSection) => s.show !== false); } catch { /* empty */ }

  const sections: TermsSection[] = customSections.length > 0
    ? customSections
    : TERMS_DEFAULTS.map((s, i) => ({ ...s, id: String(i), show: true }));

  const effectiveDate = c(cms, "terms_effective_date", "May 19, 2026");
      ...
      <div className="max-w-3xl mx-auto px-6 py-16">
        ...
        <div className="mb-10">
          <h1 className="text-4xl font-bold text-navy mb-3">
            {c(cms, "terms_headline", "Terms of Service")}
          </h1>
          <p className="text-slate-400 text-sm">Effective date: {effectiveDate}</p>
          {cms["terms_intro"] && (
            <p className="text-slate-600 mt-4 leading-relaxed">{cms["terms_intro"]}</p>
          )}
        </div>
        ...
        <div className="mt-14 pt-8 border-t border-slate-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <p className="text-slate-400 text-xs">
            {c(cms, "terms_footer_note", "Last updated: " + effectiveDate + " · These terms apply to all users of The Card Cloud.")}
          </p>
